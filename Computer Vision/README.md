...
class StereoVision:
    def __init__(self, img1_path, img2_path, K1, K2, baseline, width, height):
        # Load images for calibration
    ...
    def estimate_fundamental_matrix(self, keypoints1, keypoints2, matches):
        # Extract matched keypoints' coordinates
        pts1 = np.float32([keypoints1[m.queryIdx].pt for m in matches])
        pts2 = np.float32([keypoints2[m.trainIdx].pt for m in matches])
    ...
    def decompose_essential_matrix(self, F, pts1_inliers, pts2_inliers):
        # Decompose the Essential matrix into rotation and translation
        E = self.K2.T @ F @ self.K1
        _, self.R, self.t, _ = cv2.recoverPose(E, pts1_inliers, pts2_inliers, self.K1)
        return E

    def rectify_images(self, F, pts1_inliers, pts2_inliers):
        # Rectify images
    ...
    def compute_disparity(self, img1_rectified, img2_rectified, min_disp=0, num_disp=640, block_size=11):
        # StereoSGBM setup
        stereo = cv2.StereoSGBM_create(
            minDisparity=min_disp,
            numDisparities=num_disp,
    ...
    def save_and_display_disparity(self, disparity):
        # Normalize and save the disparity map
        disparity_rescaled = cv2.normalize(disparity, None, 0, 255, cv2.NORM_MINMAX)
        disparity_rescaled = np.uint8(disparity_rescaled)
    ...
    def compute_and_display_depth(self, disparity):
        # Calculate depth map
        focal_length = self.K1[0, 0]  # in pixels
        depth = (focal_length * self.baseline) / (disparity.astype(np.float32) + 1e-6)
...
if __name__ == "__main__":
    ...
    stereo_vision = StereoVision("new folder/2/im0.png", "new folder/2/im1.png", K1, K2, baseline, width, height)
    
    keypoints1, descriptors1, keypoints2, descriptors2 = stereo_vision.detect_features()
    matches = stereo_vision.match_features(descriptors1, descriptors2)
    F, pts1_inliers, pts2_inliers = stereo_vision.estimate_fundamental_matrix(keypoints1, keypoints2, matches)
    E = stereo_vision.decompose_essential_matrix(F, pts1_inliers, pts2_inliers)
    
    img1_rectified, img2_rectified, H1, H2 = stereo_vision.rectify_images(F, pts1_inliers, pts2_inliers)
    
    lines1 = cv2.computeCorrespondEpilines(pts2_inliers.reshape(-1, 1, 2), 2, F).reshape(-1, 3)
    lines2 = cv2.computeCorrespondEpilines(pts1_inliers.reshape(-1, 1, 2), 1, F).reshape(-1, 3)

    img1_lines, img2_lines = stereo_vision.plot_epipolar_lines(stereo_vision.img1.copy(), stereo_vision.img2.copy(),
```
